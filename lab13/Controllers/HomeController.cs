...
namespace lab13.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult Quiz(int answer, string action)
        {
            ...
            {
                QuizQuestion question = _quizService.getCurrent(quiz);
                return View("QuizQuestion", new QuizModel(quiz));
            }
            _quizService.answerLast(new QuizQuestionAnswer(answer), quiz);
            if (action == "next")
            {
                QuizQuestion question = _quizService.addNew(quiz);
                return View("QuizQuestion", new QuizModel(quiz));
            }
            return View("Result", quiz);
        }
